...
<%
	...
	else
	{
		...
		if(userType==1 || userType==3)
		{
			...
			ResultSet deptDetails = CommonServices.getDepartmentDetails(deptId);
			deptDetails.next();
%>
...
<body>
   <div class="wrapper">
    <div class="navbar">
      ...
      <div class="container" style="gap: 21rem; justify-content: flex-start;">
        	...
        	<span class="back" onclick="window.history.back()"><img src="icons/arrowBack.svg" alt="back" style="height: 30px;"></span>
                <span class="heading">Update Department</span>
            </div>
            <div id="status" style="margin-bottom: 2rem;" hidden="hidden"></div>
            <form action="<%=request.getContextPath()%>/updateDeparment" method="post">
                <div class="container2">
                    <div class="columns">
                        <div class="first-col">
                            <span class="lebel-box">Department Name</span>
                            <input type="text" class="box" id="Department" name="dept_name" placeholder="Enter Department Name" value="<%=deptDetails.getString("Dept_Name")%>" required>
                        </div>
                    </div>
                </div>
            
            <div style="padding-bottom: 1rem; text-align: center;">
                <button id="submit" type="submit" class="btn" style="position: unset;">Update Department</button>
            </div>
        </form>
        </div>
	...
	<script src="js/updateDepartmentScript.js"></script>
</body>
</html>
